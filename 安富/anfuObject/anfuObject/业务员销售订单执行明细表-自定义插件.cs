...
namespace anfuObject
{
    ...
    public class xsdd_run_entry_plugin : SalDetailRpt
    {
        ...
        public override void BuilderReportSqlAndTempTable(IRptParams filter, string tableName)
        {
            ...
            if (!checkSome)
            {
                string strSql2 = string.Format(@"select T1.* into {0} from {1} T1 join v_xsdd_zxmx_username_pwd_plugin C on T1.FSALES = C.F_CHECK_NAME where 1 = 0",
                tableName, strTable);
            ...
            string strSql = string.Format(@"select T1.*, T1.F_anfu_Text into {0} from {1} T1 join username_pwd_plugin C on T1.FSALES = C.F_CHECK_NAME and T1.FBILLNO = C.FBILLNO where ({2})",
                tableName, strTable, strFilter);
