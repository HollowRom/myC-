...
func (m *FooModel) ResetRows() {
	...
	db.Model(&Foo{}).Select("fitemid as [Index], fname as Bar, 1.23 as Baz, getdate() as Quux").Find(&itms)
...
func main() {
	dsn := "sqlserver://test:test@localhost:1433?database=AIS201407281401391"
	...
	MainWindow{
		...
		Children: []Widget{
			TextEdit{Text: "搜索条件", MaxSize: Size{
				Width:  780,
				Height: 50,
			}, MinSize: Size{
				Width:  100,
