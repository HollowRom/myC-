            ...
            MainWorker.QuequeTask(delegate
            {
                ...
                try
                {
                    ...
                    for (int row = 0; row < entryRowCount; ++row)
                    {
                        ...
                        this.View.Model.SetValue("F_VBDA_Text", ((DynamicObject)this.View.Model.GetValue("FMaterialId2", row))["Number"].ToString(), row);
                        //this.View.Model.SetValue("F_VBDA_Text", Dyobj[0]["F_VBDA_Text"].ToString(), row);
                    }
                } catch (Exception e)
                {
